...
@Component({
  selector: 'app-employeeapprovalflow',
  standalone: true,
  imports: [CommonModule, RouterModule, FormsModule],
  templateUrl: './employeeapprovalflow.component.html',
  styleUrl: './employeeapprovalflow.component.scss',
})
export class EmployeeapprovalflowComponent implements OnInit {
  formType = '';
  formId = 0;
  ...
  /** 前端群組化後的 2~3 個步驟（Agent/Dept/HR；MissingPunch 會拿掉 Agent） */
  groups: GroupStep[] = [];
  ...
  approve(): void {
    ...
    this.svc.approve(currentId, { comment: this.comment || null }).subscribe({
      next: () => { this.okMsg.set('已簽核'); this.load(); },
      error: (e) => { this.err.set(e?.error ?? '簽核失敗'); this.loading.set(false); },
    });
  }
  ...
  reject(): void {
    ...
    this.svc.reject(currentId, { comment: this.comment || null }).subscribe({
      next: () => { this.okMsg.set('已退回'); this.load(); },
      error: (e) => { this.err.set(e?.error ?? '退回失敗'); this.loading.set(false); },
    });
  }
  ...
  cancel(): void {
    ...
    this.svc.cancel(this.formType, this.formId).subscribe({
      next: () => { this.okMsg.set('已抽單'); this.load(); },
      error: (e) => { this.err.set(e?.error ?? '抽單失敗'); this.loading.set(false); },
    });
  }
  ...
  /** 群組時間軸：把多筆節點壓成 2~3 點（Agent / Dept / HR） */
  private buildGroups(d: ApprovalDetailDto | null): GroupStep[] {
    ...
    const order: GroupKey[] =
      d.formType === 'MissingPunch' ? ['Dept', 'HR'] : ['Agent', 'Dept', 'HR'];
    ...
    for (const k of order) {
      ...
      if (!list || list.length === 0) {
        // 若該群本來就沒有節點（例如你的樣板沒設定代理人），也不要顯示空點
        continue;
      }
  ...
  private labelOf(k: GroupKey): string {
    switch (k) {
      case 'Agent': return '職務代理人';
      case 'Dept': return '部門主管';
      case 'HR': return '人資覆核';
    }
  }
